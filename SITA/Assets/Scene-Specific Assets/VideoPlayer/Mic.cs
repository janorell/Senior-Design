...
public class Mic : MonoBehaviour
{
    ...
    private IEnumerator RecordingHandler()
    {
        ...
        while (true)
        {
            ...
            if (diff > bufferSize) //only do if buffer is filled
            {
                ...
                if (bReportAnalysis)
                {//report all info only for buffers with vocalization
    ...
    {
        //increment videoNum in videoPlayer
        if (++modifiedVideoPlayer.videoNum == 10)
            modifiedVideoPlayer.videoNum = 1;
        PlayerPrefs.SetInt("CurrentVideo", modifiedVideoPlayer.videoNum - 1);
        //micEnvelope
        micEnvelope.get();
        //TempMicData_NoTrim
        ...
        if (score > 47.5f)
        {
            manageScenes.ChangeScene("Perfect Win");
        }
        //better
        else if (score > 40.0f)
        {
            manageScenes.ChangeScene("Great Job");
        }
        //good
        else
        {
           manageScenes.ChangeScene("Almost There");
        }
